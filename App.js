...
Return ONLY JSON format: 
{
  ...
  function renderHome() {
    return (
      <Section title={t("home.title", { name })} subtitle={t("home.subtitle", { goal, level, plan })}>
        ...
        <Card title="🧳 7天出國旅遊挑戰" sub="目標：在 7 天內掌握機場、飯店、餐廳等核心情境。">
          <Text style={styles.body}>目前進度：{completedLessons.filter(l => l.startsWith('travel_')).length} / 7 天</Text>
          <Button label="繼續挑戰" onPress={() => setScreen("travel")} />
        </Card>
  ...
  function renderTravel() {
    const travelDays = ["travel_d1", "travel_d2", "travel_d3", "travel_d4", "travel_d5", "travel_d6", "travel_d7"];
    const completedCount = completedLessons.filter(l => travelDays.includes(l)).length;
    const progress = (completedCount / 7) * 100;

    return (
      <Section title="7天出國旅遊挑戰" subtitle="循序漸進學會旅遊必備口說與聽力">
        <Card title="🏆 學習進度" sub={`${completedCount} / 7 天已完成`}>
          <View style={{ height: 10, backgroundColor: '#e0e0e0', borderRadius: 5, marginTop: 8, overflow: 'hidden' }}>
            <View style={{ width: `${progress}%`, height: '100%', backgroundColor: '#214e63' }} />
          </View>
          <Text style={[styles.helper, { marginTop: 8 }]}>完成所有單元即可獲得旅遊行前證書！</Text>
        </Card>
        
        {travelDays.map((id, index) => {
          const lesson = dict.lessons[id];
          const isCompleted = completedLessons.includes(id);
          return (
            <Card key={id} title={lesson.title} sub={lesson.duration}>
              <Text style={styles.body}>{lesson.summary}</Text>
              <Row style={{ marginTop: 10 }}>
                <Button 
                  label={isCompleted ? "✅ 已複習" : "📖 開始學習"} 
                  onPress={() => {
                    setLessonId(id);
                    setScreen("lesson");
                  }} 
                ...
                {!isCompleted && (
                  <GhostButton 
                    label="🎯 標記完成" 
                    onPress={() => setCompletedLessons(prev => [...prev, id])} 
                  />
                )}
              </Row>
            </Card>
