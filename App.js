...
Return ONLY JSON format: {"reply": "your opening", "zh": "繁體中文翻譯"}`;
      ...
      setError("開場載入失敗：" + parseErrorMessage(e));
    }
    setLoading(false);
  }
  ...
  async function askGemini() {
    ...
    if (plan === "free" && freeCredits <= 0) {
      setError("您的免費對話次數已用完，請購買會員以繼續練習。");
      setScreen("plans");
      return;
    }
  ...
  function parseErrorMessage(err) {
    if (!err) return "未知問題，請聯絡管理員";
    // 如果 err 是字串，嘗試解析 JSON
    let errorObj = err;
    if (typeof err === "string" && err.includes("{")) {
       try { errorObj = JSON.parse(err.match(/\{[\s\S]*\}/)[0]); } catch(e) {}
    }
    ...
    if (code === 429 || status === "RESOURCE_EXHAUSTED" || message.toLowerCase().includes("quota")) {
      return "系統過載，請稍等幾秒後再試";
    }
    
    if (typeof apiError === "string") return apiError;
    return apiError.message || "未知問題，請聯絡管理員";
  }
...
Return ONLY JSON format: {"reply": "your conversation response", "zh": "繁體中文翻譯"}`;
      ...
      setError("連線伺服器錯誤：" + parseErrorMessage(e));
    }
    setLoading(false);
  }
...
Return ONLY JSON format: 
{
  ...
  "extendedPhrases": [{"en": "phrase", "zh": "翻譯"}]
      ...
      setError("結算分析失敗：" + parseErrorMessage(e));
    } finally {
      setLoading(false);
    }
  }
